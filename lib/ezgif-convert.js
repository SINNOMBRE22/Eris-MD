...
async function render(fields) {
    ...
    html.data.toString().split('(drag and drop frames to change order)')[1].split('<p class="options"><strong>Toggle a range of frames:</strong>')[0].split('<span class="frame-tools">').slice(0, -1).map(e => {
        fields['files[]'].push(e)
    });
    let image = await axios({
        method: 'post',
