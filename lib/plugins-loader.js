...
function pathToFileURL(p) {
  // fallback if global.__filename not present
...
export default async function initPlugins(pluginRoot) {
  ...
  for (const rel of pluginFiles) {
    ...
    try {
      ...
      console.error(`Error cargando plugin ${rel}:`, e)
      // keep going
    }
  }
  ...
  global.reload = async (_ev, filename) => {
    ...
    if (!existsSync(full)) {
      if (rel in global.plugins) {
        ...
        console.log(`plugin eliminado: ${rel}`)
      }
      return
    }
    ...
    try {
      ...
      if (err) {
        console.error(`Error de sintaxis en plugin ${rel}:\n`, format(err))
        return
      }
    } catch (e) {
      console.error('No se pudo leer el plugin para verificar sintaxis:', e)
    }
    ...
    try {
      ...
      console.log(`plugin recargado: ${rel}`)
    } catch (e) {
      console.error(`Error recargando plugin ${rel}:`, e)
    }
  }
  ...
  try {
    ...
    console.log('Watcher en plugins (recursive) activo en:', pluginRootAbs)
  } catch (e) {
    // fallback: create watchers per directory
    createDirWatchers(pluginRootAbs, (ev, rel) => {
      global.reload(ev, rel)
    })
    console.log('Watcher en plugins activo (por carpetas) en:', pluginRootAbs)
  }
