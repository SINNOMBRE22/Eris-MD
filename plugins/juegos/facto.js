/* ERIS-MD FACTOS (ROASTS) */

let handler = async (m, { conn }) => {
    const senderName = m.pushName || (await conn.getName(m.sender)) || "Usuario";

    // Array local: Cero lag y no consume memoria en segundo plano
    const factos = [
    ...
    const facto = factos[Math.floor(Math.random() * factos.length)];

    let caption = `╭─── [ 🔥 *FACTO* ] ──···\n`;
    caption += `│ 🗣️ "${facto}"\n`;
    caption += `╰─────────────────────────···\n\n`;
    caption += `> 🌸 *Sin piedad, ${senderName}.*`;
...
handler.help = ['facto'];
handler.tags = ['juegos'];
handler.command = ['facto', 'factos', 'verdad'];
handler.group = true;
