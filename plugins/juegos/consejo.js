/* ERIS-MD CONSEJOS RANDOM */

let handler = async (m, { conn }) => {
    // Array local: Solo se carga en memoria cuando se ejecuta el comando
    const consejos = [
        ...
        "Puedes más de lo que te imaginas, vales más de lo que crees (Anónimo)",
        ...
        "Todas las mañanas, levántate con la idea de comerte el mundo (Anónimo)",
        "No cuentes los días, haz que los días cuenten (Anonimo)"
    ];

    // Selección aleatoria limpia
    const frase = consejos[Math.floor(Math.random() * consejos.length)];

    let caption = `╭─── [ 🧠 *CONSEJO DEL DÍA* ] ──···\n`;
    caption += `│ 💬 "${frase}"\n`;
    caption += `╰─────────────────────────···\n\n`;
    const nombre = m.pushName || "Usuario";
caption += `> 🌸 *Palabras para ti, ${nombre}.*`;
...
handler.tags = ['juegos']; // O puedes ponerle ['frases'] si planeas hacer una categoría propia
handler.command = ['consejo', 'frase', 'motivacion'];
handler.group = true;
